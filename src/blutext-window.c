...
save_file (BlutextWindow *self, GFile *file)
{
  ...
  g_file_replace_contents_bytes_async (file, bytes, NULL,
...
static void
blutext_window__save_file_dialog (GAction *action,
                                  GVariant *param,
                                  BlutextWindow *self)
{
  g_autoptr (GtkFileDialog) dialog = gtk_file_dialog_new ();
...
blutext_window_init (BlutextWindow *self)
{
  ...
  g_autoptr (GSimpleAction) save_as_action = g_simple_action_new ("save-as",
                                                                  NULL);
